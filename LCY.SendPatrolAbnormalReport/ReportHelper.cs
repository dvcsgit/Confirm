...
using LCY.SendPatrolAbnormalReport.Models;
using System;
...
namespace LCY.SendPatrolAbnormalReport
{
    public class ReportHelper : IDisposable
    {
        public void Send()
        {
            ...
            {
                var downStream = OrganizationDataAccessor.GetDownStreamOrganizationList("960f531a-463b-4f44-812c-dbc938950ef0", true);

                var yesterday = DateTime.Today.AddDays(-1);
                var today = DateTime.Today;

                var b = new DateTime(yesterday.Year, yesterday.Month, yesterday.Day, 8, 0, 0);
                var begin = b.ToString("yyyyMMddHHmmss");
                Logger.Log(string.Format("Begin:{0}", begin));

                var e = new DateTime(today.Year, today.Month, today.Day, 8, 0, 0);
                var end = e.ToString("yyyyMMddHHmmss");
                ...
                var jobResultList = (from x in db.JobResult
                                     ...
                                     where downStream.Contains(r.OrganizationUniqueID) && string.Compare(x.BeginDate + x.BeginTime + "00", begin) > 0 && DateTime.Compare(x.JobEndTime, e) <= 0
                                     select new
                                     {
