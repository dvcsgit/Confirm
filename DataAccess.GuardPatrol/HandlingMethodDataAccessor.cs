            ...
            try
            {
                ...
                using (GDbEntities edb = new GDbEntities())
                {
                    if (string.IsNullOrEmpty(HandlingMethodType))
                    {
                        ...
                        {
                            var handlingMethodTypeList = edb.HandlingMethod.Where(x => x.OrganizationUniqueID == OrganizationUniqueID).Select(x => x.HandlingMethodType).Distinct().OrderBy(x => x).ToList();
                        ...
                        using (DbEntities db = new DbEntities())
                        {
                            ...
                            foreach (var organization in organizationList)
                            {
                                ...
                                if (db.Organization.Any(x => x.ParentUniqueID == organization.UniqueID && Account.VisibleOrganizationUniqueIDList.Contains(x.UniqueID)) 
                                    ||
                                    (Account.QueryableOrganizationUniqueIDList.Contains(organization.UniqueID) && edb.HandlingMethod.Any(x => x.OrganizationUniqueID == organization.UniqueID)))
                                {
                                    treeItem.State = "closed";
                                }
                    ...
                    {
                        var handlingMethodList = edb.HandlingMethod.Where(x => x.OrganizationUniqueID == OrganizationUniqueID && x.HandlingMethodType == HandlingMethodType).OrderBy(x => x.ID).ToList();
                    ...
                    if (string.IsNullOrEmpty(HandlingMethodType))
                    {
                        ...
                        using (DbEntities db = new DbEntities())
                        {
                            ...
                            foreach (var organization in organizationList)
                            {
                                ...
                                if (edb.HandlingMethod.Any(x => downStreamOrganizationList.Contains(x.OrganizationUniqueID) && availableOrganizationList.Contains(x.OrganizationUniqueID)))
                                {
                                    var treeItem = new TreeItem() { Title = organization.Description };
