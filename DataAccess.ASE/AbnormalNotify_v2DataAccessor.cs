...
{
    public class AbnormalNotify_v2DataAccessor
    {
        public static RequestResult GetQueryFormModel(string Status, List<Organization> ResponsibleOrganizationList, List<Organization> OrganizationList, List<Models.Shared.UserModel> AccountList)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(Status))
                {
                    ...
                    model = new QueryFormModel()
                    {
                        ...
                        {
                            OccurBeginDateString = DateTimeHelper.DateTime2DateStringWithSeperator(new DateTime(DateTime.Today.Year, 1, 1))
                        }
                    };
        ...
        public static RequestResult Query(QueryParameters Parameters,List<Models.Shared.Organization> OrganizationList, List<Models.Shared.UserModel> UserList)
        {
            ...
            try
            {
                ...
                using (ASEDbEntities db = new ASEDbEntities())
                {
                    var replyToUserList = (from x in db.FAC_ABNORMALNOTIFYGROUP
                                          join y in db.FAC_ABNORMALNOTIFYGROUPUSER
                                          on x.UNIQUEID equals y.GROUPUNIQUEID
                                          join u in db.ACCOUNT
                                          on y.USERID equals u.ID
                                          where x.GROUPTYPE == "2"
                                          select u.ID).ToList();

                    var replyCCUserList = (from x in db.FAC_ABNORMALNOTIFYGROUP
                                            join y in db.FAC_ABNORMALNOTIFYGROUPCCUSER
                                            on x.UNIQUEID equals y.GROUPUNIQUEID
                                            join u in db.ACCOUNT
                                            on y.USERID equals u.ID
                                            where x.GROUPTYPE == "2"
                    ...
                    model.ReplyUserList=replyToUserList.Union(replyCCUserList).Distinct().ToList();

                    var query = db.FAC_ABNORMALNOTIFYFORM.AsQueryable();

                    if (!string.IsNullOrEmpty(Parameters.OccurBeginDate))
                    {
                        query = query.Where(x => string.Compare(x.OCCURDATE, Parameters.OccurBeginDate) >= 0);
                    }

                    if (!string.IsNullOrEmpty(Parameters.OccurEndDate))
                    {
                        query = query.Where(x => string.Compare(x.OCCURDATE, Parameters.OccurEndDate) <= 0);
                    }
                    ...
                    if (!string.IsNullOrEmpty(Parameters.ResOrganizationUniqueID))
                    {
                        query = query.Where(x => x.RESORGANIZATIONUNIQUEID == Parameters.ResOrganizationUniqueID);
                    }

                    if (Parameters.ClosedBeginDate.HasValue)
                    {
                        query = query.Where(x => x.CLOSEDTIME.HasValue && DateTime.Compare(x.CLOSEDTIME.Value, Parameters.ClosedBeginDate.Value) >= 0);
                    }

                    if (Parameters.ClosedEndDate.HasValue)
                    {
                        query = query.Where(x => x.CLOSEDTIME.HasValue && DateTime.Compare(x.CLOSEDTIME.Value, Parameters.ClosedEndDate.Value) <= 0);
                    }
        ...
        public static RequestResult GetReplyFormModel(string UniqueID, List<Models.Shared.Organization> OrganizationList, List<Models.Shared.UserModel> UserList)
        {
            RequestResult result = new RequestResult();
        ...
        public static void SendNotifyMail(string UniqueID, bool IsResend)
        {
            try
            {
                ...
                using (ASEDbEntities db = new ASEDbEntities())
                {
                    ...
                    var groupList = (from x in db.FAC_ABNORMALNOTIFYFORMGROUP
                                     join g in db.FAC_ABNORMALNOTIFYGROUP
                                     on x.GROUPUNIQUEID equals g.UNIQUEID
                                     where x.FORMUNIQUEID == form.UNIQUEID && g.GROUPTYPE == "1"
                                     select x.GROUPUNIQUEID).ToList();
        ...
        public static void SendReplyMail(string UniqueID, bool IsResend)
        {
            try
            {
                var mailAddressList = new List<MailAddress>();
                ...
                using (ASEDbEntities db = new ASEDbEntities())
                {
                    ...
                    var groupList = db.FAC_ABNORMALNOTIFYGROUP.Where(x => x.GROUPTYPE == "2").Select(x => x.UNIQUEID).ToList();
                    ...
                    if (mailAddressList.Count > 0)
                    {
                        ...
                        if (!IsResend)
                        {
                            subject = string.Format("[災損提報][{0}]{1}", form.VHNO, form.SUBJECT);
                        }
                        else
                        {
                            subject = string.Format("【更新】[災損提報][{0}]{1}", form.VHNO, form.SUBJECT);
                        }
