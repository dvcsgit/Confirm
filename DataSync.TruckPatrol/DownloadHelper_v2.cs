...
{
    public class DownloadHelper_v2 : IDisposable
    {
        private string Guid;

        private DownloadDataModel DataModel = new DownloadDataModel();

        public RequestResult Generate(string UserID, List<string> TruckUniqueIDList)
        {
            ...
            try
            {
                ...
                if (result.IsSuccess)
                {
                    result = Query(UserID, TruckUniqueIDList);
        ...
        private RequestResult Query(string UserID, List<string> TruckUniqueIDList)
        {
            RequestResult result = new RequestResult();
            ...
            try
            {
                using (TDbEntities db = new TDbEntities())
                {
                    ...
                    #region Truck
                    var truckList = db.Truck.Where(x => TruckUniqueIDList.Contains(x.UniqueID)).ToList();
                    ...
                    foreach (var truck in truckList)
                    {
                        var truckModel = new TruckModel()
                        {
                            ...
                            Remark = truck.Remark,
                            LastModifyTime = DateTimeHelper.DateTime2DateTimeString(truck.LastModifyTime)
                        };
                ...
                using (DbEntities db = new DbEntities())
                {
                    ...
                    var ancestorOrganizationUniqueID = OrganizationDataAccessor.GetAncestorOrganizationUniqueID(user.OrganizationUniqueID);
                    ...
                    using (SQLiteTransaction trans = conn.BeginTransaction())
                    {
                        ...
                        foreach (var truck in DataModel.TruckList)
                        {
                            ...
                            foreach (var controlPoint in truck.ControlPointList)
                            {
                                using (SQLiteCommand cmd = conn.CreateCommand())
                                {
                                    cmd.CommandText = "INSERT INTO ControlPoint (TruckUniqueID, ControlPointUniqueID, ID, Description, IsFeelItemDefaultNormal, TagID, Remark, Seq, MinTimeSpan) VALUES (@TruckUniqueID, @ControlPointUniqueID, @ID, @Description, @IsFeelItemDefaultNormal, @TagID, @Remark, @Seq, @MinTimeSpan)";
