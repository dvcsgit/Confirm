...
using DataAccess;
using Report.EquipmentMaintenance.Models.UnArriveResultAnalyze;
using DataAccess.EquipmentMaintenance;

namespace Report.EquipmentMaintenance.DataAccess
{
    public class UnArriveResultAnalyzeDataAccessor
    {
        public static RequestResult Query(QueryParameters Parameters, Account Account)
        {
            ...
            try
            {
                using (EDbEntities db = new EDbEntities())
                {
                    ...
                    var arriveRecordModelGroupByUserID = (from x in arriveRecordModelList group x by x.UserID into g select new { g.Key, g  }).ToList();

                    foreach (var arriveRecordModel in arriveRecordModelGroupByUserID)
                    {
        ...
        public static ExcelExportModel Export(GridViewModel Model, Define.EnumExcelVersion ExcelVersion)
        {
            try
            {
                ...
                ISheet sheet = wk.CreateSheet(Resources.Resource.UnArriveResultAnalyze);
                ...
                foreach (var item in Model.ItemList)
                {
                    ...
                    cell.SetCellValue(Resources.Resource.CheckUser);  //检查人员
                ...
                var model = new ExcelExportModel(Resources.Resource.UnArriveResultAnalyze, ExcelVersion);
