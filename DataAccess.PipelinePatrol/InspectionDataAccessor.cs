...
{
    public class InspectionDataAccessor
    {
        ...
        public static RequestResult Query(QueryParameters Parameters)
        {
            ...
            try
            {
                using (PDbEntities db = new PDbEntities())
                {
                    var query = (from i in db.Inspection
                                 join f in db.ConstructionFirm
                                 on i.ConstructionFirmUniqueID equals f.UniqueID into tmpFirm
                                 from f in tmpFirm.DefaultIfEmpty()
                                 join t in db.ConstructionType
                                 on i.ConstructionTypeUniqueID equals t.UniqueID into tmpType
            ...
            try
            {
                using (PDbEntities db = new PDbEntities())
                {
                    var query = (from i in db.Inspection
                                 ...
                                 on i.PipePointUniqueID equals p.UniqueID into tmpPipePoint
                    ...
                    var photoList = db.InspectionPhoto.Where(x => x.InspectionUniqueID == UniqueID).OrderBy(x => x.Seq).ToList();

                    foreach (var photo in photoList)
                    {
                        model.PhotoList.Add(string.Format("{0}_{1}.{2}", photo.InspectionUniqueID, photo.Seq, photo.Extension));
                    }

                    var dialog = db.Dialog.FirstOrDefault(x => x.InspectionUniqueID == UniqueID);
