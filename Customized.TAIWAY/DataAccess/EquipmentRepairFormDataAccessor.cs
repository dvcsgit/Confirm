...
using Customized.TAIWAY.Models.EquipmentRepairForm;
...
namespace Customized.TAIWAY.DataAccess
{
  public   class EquipmentRepairFormDataAccessor
    {
        ...
        public static ExcelExportModel Export(GridViewModel Model, Define.EnumExcelVersion ExcelVersion)
        {
            ...
            try
            {
                ...
                sheet.SetColumnWidth(0, 5 * 256);     //设置单元格长度
                ...
                sheet.AddMergedRegion(new CellRangeAddress(2, 2, 6, 7));  //合并列印日期单元格
                ...
                sheet.AddMergedRegion(new CellRangeAddress(4, 7, 5, 5));
                sheet.AddMergedRegion(new CellRangeAddress(4, 7, 6, 7));
                sheet.AddMergedRegion(new CellRangeAddress(4, 7, 8, 8));
                ...
                //創建填充查詢條件數據
                sheet.GetRow(2).GetCell(6).SetCellValue(Resources.Resource.PrintDate + ":");
                sheet.GetRow(2).GetCell(8).SetCellValue(DateTime.Now.ToString("yyyy/MM/dd"));

                //創建填充資料
                sheet.GetRow(3).GetCell(1).SetCellValue(Resources.Resource.EquipUniqueID + "：");
                sheet.GetRow(3).GetCell(3).SetCellValue(Model.Export.EquipmentID);
                sheet.GetRow(3).GetCell(5).SetCellValue(Resources.Resource.ManageUserID);
                sheet.GetRow(3).GetCell(6).SetCellValue(Resources.Resource.Engineer);
                sheet.GetRow(3).GetCell(8).SetCellValue(Resources.Resource.ImplementUser);


                sheet.GetRow(4).GetCell(1).SetCellValue(Resources.Resource.EquipmentName + "：");
                sheet.GetRow(4).GetCell(3).SetCellValue(Model.Export.EquipmentName);
                sheet.GetRow(4).GetCell(5).SetCellValue(Model.Export.ManagerUserID);
                sheet.GetRow(4).GetCell(6).SetCellValue(Model.Export.Engineer);
                sheet.GetRow(4).GetCell(8).SetCellValue(Model.Export.ImplementUser);
                ...
                sheet.GetRow(8).GetCell(4).SetCellValue(Resources.Resource.StandardItem);
                sheet.GetRow(8).GetCell(6).SetCellValue(Resources.Resource.State);
                sheet.GetRow(8).GetCell(7).SetCellValue(Resources.Resource.Deal);
                int row = 9;
                foreach (var export in Model.ExportList)
                {
                    ...
                    sheet.AddMergedRegion(new CellRangeAddress(row, row, 7, 8));
                ...
                sheet.AddMergedRegion(new CellRangeAddress(row + 1, row + 9, 1, 1));
                sheet.AddMergedRegion(new CellRangeAddress(row + 1, row + 9, 2, 8));

                sheet.GetRow(row + 1).GetCell(1).SetCellValue(Resources.Resource.Tips);
                sheet.AddMergedRegion(new CellRangeAddress(row + 10, row + 10, 6, 7));

                sheet.GetRow(row + 10).GetCell(6).SetCellValue(Resources.Resource.Fieldcadre);
                sheet.GetRow(row + 10).GetCell(6).CellStyle.SetFont(font);

                for (var i = 3; i < row + 10; i++)
                {
                    for (var j = 1; j < 9; j++)
                    {
